...
class CategoryType(Base):
    """
    Model representing category types in the database.
    
    Attributes:
        code: Primary key, unique identifier for the category type (max 5 chars)
        name: Human-readable name for the category type
    ...
    __table_args__ = (
        Index('ix_category_types_name_code', 'name', 'code'),
    )
    
    def __repr__(self):
        """String representation of the category type"""
        return f"<CategoryType(code='{self.code}', name='{self.name}')>"
