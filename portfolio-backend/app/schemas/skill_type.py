...
class Filter(BaseModel):
    """Generic schema for filtering skill types"""
    field: str
...
# --- SkillType Specific Schemas ---

class SkillTypeBase(BaseModel):
    """Base schema for skill type data"""
    code: str = Field(..., min_length=1, max_length=5, description="Unique code for the skill type (max 5 chars)")
    name: str = Field(..., min_length=1, max_length=100, description="Human-readable name for the skill type")

    @field_validator('code')
    @classmethod
    def code_must_not_be_empty(cls, v: str) -> str:
        if not v.strip():
            raise ValueError('Skill type code cannot be empty or whitespace.')
    ...
    def name_must_not_be_empty(cls, v: str) -> str:
        if not v.strip():
            raise ValueError('Skill type name cannot be empty or whitespace.')
        return v.strip()  # Trim whitespace

class SkillTypeCreate(SkillTypeBase):
    """Schema for creating a new skill type"""
    # Inherits fields and validation from SkillTypeBase
    pass

class SkillTypeUpdate(BaseModel):
    """Schema for updating an existing skill type. All fields are optional."""
...
class SkillType(SkillTypeBase):
    """Schema for skill type with ORM mode configuration"""
    model_config = ConfigDict(from_attributes=True)

class SkillTypeOut(SkillType):
    """Schema for representing a skill type in API responses"""
    pass

# Specific Paginated Response for SkillTypes using the generic schema
PaginatedSkillTypeResponse = PaginatedResponse[SkillTypeOut] 