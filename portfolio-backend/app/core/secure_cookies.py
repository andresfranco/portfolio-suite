...
class SecureCookieManager:
    ...
    def verify_csrf_token(cls, request: Request) -> bool:
        ...
        Uses the double-submit cookie pattern: the CSRF cookie value must match
        the X-CSRF-Token header value.  This is sufficient for CSRF protection
        because the same-origin policy prevents cross-origin JavaScript from
        reading cookies, so an attacker on a different domain cannot obtain the
        cookie value to forge the matching header.

        NOTE: The in-memory token store (_csrf_tokens) is intentionally NOT used
        for validation here.  With multiple uvicorn workers each process has its
        own memory space, so a token generated by Worker A would not be found in
        Worker B's store, causing spurious 403 failures on every other request.
        The cookie/header match alone provides the required CSRF protection.
