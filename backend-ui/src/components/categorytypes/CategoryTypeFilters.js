...
const CategoryTypeFilters = () => {
  const { filters, updateFilters, clearFilters, fetchCategoryTypes } = useCategoryType();
  ...
  useEffect(() => {
    logInfo(`CategoryTypeFilters (${instanceIdRef.current}) - Current filters:`, filters);
  }, [filters]);
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(prevFiltersRef.current) !== JSON.stringify(filters)) {
      logInfo(`CategoryTypeFilters (${instanceIdRef.current}) - External filters changed:`, filters);
  ...
  const onSubmit = (data) => {
    logInfo(`CategoryTypeFilters (${instanceIdRef.current}) - Submitting filters:`, data);
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (value && typeof value === 'string' && value.trim() !== '') {
        ...
        logInfo(`CategoryTypeFilters - Adding filter ${key}: "${value.trim()}"`);
      }
    });
    
    logInfo(`CategoryTypeFilters (${instanceIdRef.current}) - Applied filters:`, newFilters);
    ...
    // Explicitly fetch category types with the new filters
    // This is the key addition that ensures the grid refreshes
    fetchCategoryTypes(0, 10, newFilters);
  };

  // Clear all filters
  const handleClearFilters = () => {
    logInfo(`CategoryTypeFilters (${instanceIdRef.current}) - Clearing all filters`);
    ...
    // Explicitly fetch category types with empty filters
    // This ensures the grid refreshes immediately
    fetchCategoryTypes(0, 10, {});
  };
  ...
  const handleFilterTypeChange = (filterId, newType) => {
    try {
      ...
      logInfo(`CategoryTypeFilters - Changing filter type from ${oldType} to ${newType}`);
      ...
      // Explicitly fetch category types to refresh the grid
      fetchCategoryTypes(0, 10, updatedFilters);
    } catch (error) {
  ...
  const handleRemoveFilter = (filterId) => {
    try {
      ...
      fetchCategoryTypes(0, 10, currentFilters);
    } catch (error) {
...
export default CategoryTypeFilters; 