...
const SkillTypeForm = ({ mode = 'create', initialData = null, onClose }) => {
  ...
  const { createSkillType, updateSkillType, deleteSkillType, checkCodeExists } = useSkillType();
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      if (mode === 'create') {
        logInfo('Creating skill type:', apiData);
        await createSkillType(apiData);
        setApiSuccess('Skill type created successfully!');
        reset({ code: '', name: '' });
        setTimeout(() => {
        ...
        logInfo(`Updating skill type ${initialData.code}:`, updatePayload);
        await updateSkillType(initialData.code, updatePayload);
        setApiSuccess('Skill type updated successfully!');
        setTimeout(() => {
          if (onClose) onClose(true);
        }, 1500);
      } else if (mode === 'delete' && initialData?.code) {
        logInfo(`Deleting skill type ${initialData.code}`);
        await deleteSkillType(initialData.code);
        setApiSuccess('Skill type deleted successfully!');
        setTimeout(() => {
          if (onClose) onClose(true);
        }, 1500);
      }
      ...
      <Box sx={{ p: 2 }}>
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            ...
            >
              This action cannot be undone and may affect skills using this type.
            </Typography>
