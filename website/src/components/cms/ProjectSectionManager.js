...
const SectionEditorDialog = ({ projectId, section, authToken, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Always use TipTap serialized HTML from editor onChange.
    // Reading `.ProseMirror.innerHTML` stores node-view UI controls (e.g. "EditDelete")
    // and breaks code block re-parsing after save.
    let currentHtml = formData.section_texts?.[0]?.text || '';
    currentHtml = currentHtml.replace(/\s+data-borderless-cell="[^"]*"/gi, '');
    currentHtml = currentHtml.replace(/\s+data-borderless-cell='[^']*'/gi, '');
    currentHtml = currentHtml.replace(/\s+class="([^"]*?)\btable-cell-droppable\b([^"]*)"/gi, (match, before, after) => {
